...
s
    ...
    #mean_all = np.mean(ipoled_snips)
    #print("mean all")
    #print(mean_all)

    # create sine to center data on.
    #mean = np.zeros(int(ipoled_snips.shape[1]))
    #mean[int(ipoled_snips.shape[1]/4)+1:int(ipoled_snips.shape[1]*3/4)+1] = -np.sin(0.025*np.arange(int(ipoled_snips.shape[1]/2)))
    #mean = mean + mean_all
...
def alignlabels(labels,clusters,old_labels,old_clusters,maxlabel):
    ...
    for curlabel, cluster in enumerate(clusters):
        ...
        if np.min(n) < 0.1:
            #labels[labels==np.argmin(n)] = np.max(labels) + 1
            print("replacing x with y")
            print(curlabel)
            print(old_labels[np.argmin(n)])
...
def analyze_pulse_data(filepath, deltat=10, thresh=0.04, starttime = 0, endtime = 0, savepath = False,save=False, npmmp = False, plot_steps=False, plot_result=False):
        ...
        for idx in range(0, blockamount):
            ...
            if len(pk) > 3:
                ...
                if len(peaks) > 0:
                    ...
                    labels, clusters = cluster_events(feats, peaks, 0.1, minpeaks, False, method = 'DBSCAN')
                    ...
                    for lab, color in zip(np.unique(labels),colors):
                        ...
                        if lab == -1:
                            ...
                            c=color
                            z=1
                        plt.plot(range(feats.shape[1]),np.transpose(feats[labels == lab]),color=c,zorder=z,label=lab)
